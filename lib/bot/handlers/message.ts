...
export async function handleMessage(ctx: Context) {
  ...
  if (error) {
    ...
    if (!savedFeedback) {
      console.error("No savedFeedback returned, skipping AI processing");
      return;
    }

    if (!text || text.trim().length === 0) {
      console.log("No text content in feedback, skipping AI processing");
      return;
    }
    ...
    if (projectError) {
      console.error("Error fetching project for AI processing:", projectError);
      return;
    }

    if (!project) {
      console.error(`Project ${projectId} not found, skipping AI processing`);
      return;
    }

    console.log(`Starting AI processing for feedback ${savedFeedback.id} on project ${projectId}`);
    ...
    Promise.all([
      ...
      .then(async ([scores, embedding]) => {
        console.log(`AI processing complete for feedback ${savedFeedback.id}`);
        console.log(`Scores:`, scores);
        console.log(`Embedding generated:`, !!embedding);

        if (!scores) {
          console.error("Failed to analyze feedback - no scores returned");
          return;
        }
        ...
        if (embedding) {
          // Store the embedding
          const { error: embeddingError } = await updateFeedbackEmbedding(savedFeedback.id, embedding);
          if (embeddingError) {
            console.error("Error updating feedback embedding:", embeddingError);
          } else {
            console.log(`Embedding stored for feedback ${savedFeedback.id}`);
          }
        ...
        const { error: scoresError } = await updateFeedbackScores(savedFeedback.id, {
        ...
        if (scoresError) {
          console.error("Error updating feedback scores:", scoresError);
        } else {
          console.log(`Scores updated for feedback ${savedFeedback.id}`);
        }
