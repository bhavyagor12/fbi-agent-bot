...
export const TIER_THRESHOLDS: TierConfig[] = [
    { name: 'bronze', minXP: 0, maxXP: 999, label: 'Bronze - Beginner / Newcomer' },
    { name: 'silver', minXP: 1000, maxXP: 4999, label: 'Silver - Active Contributor' },
    { name: 'gold', minXP: 5000, maxXP: 11999, label: 'Gold - Reliable Builder / Creator' },
    { name: 'platinum', minXP: 12000, maxXP: 24999, label: 'Platinum - Top 10–15%' },
    { name: 'diamond', minXP: 25000, maxXP: Infinity, label: 'Diamond - Top 1–3% (elite)' }
];
...
export const XP_CONSTANTS = {
    PROJECT_BASE: 100,           // Fixed XP for creating a project
    FEEDBACK_MIN: 5,             // Minimum XP for any feedback
 ...
 * @returns XP amount (5-100 points)
 */
export function calculateFeedbackXP(scores: {
    ...
    const { relevance, depth, evidence, constructiveness, tone, originality } = scores;

    // Calculate average score (all scores are 1-10)
    const average = (relevance + depth + evidence + constructiveness + tone + originality) / 6;

    // Convert to XP scale (1-10 average -> 10-100 XP)
    let xp = average * 10;
...
export function getTierProgress(xp: number): {
    ...
    const nextTierConfig = currentIndex < TIER_THRESHOLDS.length - 1
