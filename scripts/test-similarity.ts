...
import { generateEmbedding, cosineSimilarity, calculateOriginalityScore, calculateTimeWeight } from '../lib/embeddings';
...
async function testSimilarity() {
    ...
    for (const testCase of testCases) {
        ...
        try {
            ...
            console.log(`  Similarity Score: ${similarity.toFixed(4)} (0 = different, 1 = identical)`);
            ...
            const isGood = checkExpectation(originality, testCase.expectedOriginality);
    ...
    const similarities = [0.90, 0.85, 0.80, 0.75]; // 4 similar feedback
    const dates = [
    ...
    for (let i = 0; i < similarities.length; i++) {
        console.log(`  ${i + 1}. Similarity: ${similarities[i].toFixed(2)}, Age: ${dates[i].toLocaleDateString()}, Weight: ${weights[i].toFixed(3)}`);
    }
    console.log();
    ...
    console.log(`Weighted Originality Score: ${weightedOriginality}/10 (considers recency)`);
    console.log(`Uniform Originality Score: ${uniformOriginality}/10 (ignores recency)`);
    console.log();
    console.log('Note: Weighted scoring gives more importance to recent feedback,');
    console.log('      so similar recent feedback lowers the originality score more.');
